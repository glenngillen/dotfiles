...
const platform_1 = require("../core/common/platform");
...
`, ['.foam', 'templates', 'template-with-path.md']);
    ...
    describe('Creation with active text selection', () => {
        it('should populate FOAM_SELECTED_TEXT with the current selection', async () => {
            ...
            expect(await resolver.resolveFromName('FOAM_SELECTED_TEXT')).toEqual('first file');
            await test_utils_vscode_1.deleteFile(templateA);
            await test_utils_vscode_1.deleteFile(target);
        ...
        it('should replace selection with a link to the newly created note', async () => {
            ...
            expect(vscode_1.window.visibleTextEditors[0].document.getText()).toEqual(`This is my first file: [[${target.getName()}]]`);
        });
    });
});
describe('determineNewNoteFilepath', () => {
    ...
    it('should compute the relative template filepath from the current directory', async () => {
        const relativePath = platform_1.isWindows
        ...
        const expectedPath = vsc_utils_1.fromVsCodeUri(vscode_1.workspace.workspaceFolders[0].uri).joinPath(relativePath);
    ...
    it('should use the note title if nothing else is available', async () => {
        ...
        const expectedPath = vsc_utils_1.fromVsCodeUri(vscode_1.workspace.workspaceFolders[0].uri).joinPath(`${noteTitle}.md`);
        expect(resultFilepath.toFsPath()).toMatch(expectedPath.toFsPath());
    });
