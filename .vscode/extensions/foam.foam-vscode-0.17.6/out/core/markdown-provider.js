...
const getTextFromChildren = (root) => {
    let text = '';
    unist_util_visit_1.default(root, node => {
...
const handleError = (plugin, fnName, uri, e) => {
    ...
    log_1.Logger.warn(`Error while executing [${fnName}] in plugin [${name}]. ${uri ? 'for file [' + uri.toString() : ']'}.`, e);
};
function createMarkdownParser(extraPlugins) {
    ...
    const foamParser = {
        parse: (uri, markdown) => {
            log_1.Logger.debug('Parsing:', uri.toString());
            ...
            unist_util_visit_1.default(tree, node => {
                ...
                if (node.type === 'yaml') {
                    try {
                        ...
                        log_1.Logger.warn(`Error while parsing YAML for [${uri.toString()}]`, e);
                    }
                }
                for (let i = 0, len = plugins.length; i < len; i++) {
                    try {
