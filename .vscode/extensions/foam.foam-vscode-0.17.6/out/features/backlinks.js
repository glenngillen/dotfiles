    ...
    getChildren(item) {
        ...
        if (item) {
            ...
            const backlinkRefs = Promise.all(resource.links
                .filter(link => this.workspace.resolveLink(resource, link).isEqual(uri))
                .map(async (link) => {
                var _a;
        ...
        const backlinksByResourcePath = lodash_1.groupBy(this.graph.getConnections(uri).filter(c => c.target.isEqual(uri)), b => b.source.path);
