...
async function getWebviewContent(context, panel) {
    const datavizPath = vscode.Uri.joinPath(vscode.Uri.file(context.extensionPath), 'static', 'dataviz');
    const getWebviewUri = (fileName) => panel.webview.asWebviewUri(vscode.Uri.joinPath(datavizPath, fileName));
    const indexHtml = await vscode.workspace.fs.readFile(vscode.Uri.joinPath(datavizPath, 'index.html'));
