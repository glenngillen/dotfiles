...
class NavigationProvider {
    ...
    provideDefinition(document, position) {
        ...
        const targetRange = section
            ? section.range
            : range_1.Range.createFromPosition(targetResource.source.contentStart, targetResource.source.end);
        const targetSelectionRange = section
            ? section.range
            : range_1.Range.createFromPosition(targetRange.start);
        const result = {
            originSelectionRange: vsc_utils_1.toVsCodeRange(targetLink.range),
            targetUri: vsc_utils_1.toVsCodeUri(uri.asPlain()),
            targetRange: vsc_utils_1.toVsCodeRange(targetRange),
            targetSelectionRange: vsc_utils_1.toVsCodeRange(targetSelectionRange),
        };
        return [result];
    }
    /**
