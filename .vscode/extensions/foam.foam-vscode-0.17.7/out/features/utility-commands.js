...
const feature = {
    activate: (context, foamPromise) => {
        context.subscriptions.push(vscode.commands.registerCommand(exports.OPEN_COMMAND.command, async (params) => {
            var _a, _b, _c;
            const uri = new uri_1.URI(params.uri);
            switch (uri.scheme) {
                case 'file': {
                    const targetUri = uri.path === ((_a = vscode.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.document.uri.path)
                        ? (_b = vscode.window.activeTextEditor) === null || _b === void 0 ? void 0 : _b.document.uri : vsc_utils_1.toVsCodeUri(uri.asPlain());
                    const targetEditor = vscode.window.visibleTextEditors.find(ed => targetUri.path === ed.document.uri.path);
                    const column = targetEditor === null || targetEditor === void 0 ? void 0 : targetEditor.viewColumn;
                ...
                case 'placeholder': {
                    const basedir = vscode.workspace.workspaceFolders.length > 0
                        ...
                        : ((_c = vscode.window.activeTextEditor) === null || _c === void 0 ? void 0 : _c.document.uri) ? vscode.window.activeTextEditor.document.uri
                            : undefined;
                    if (basedir === undefined) {
                        return;
                    }
                    const title = uri.getName();
                    const target = vsc_utils_1.fromVsCodeUri(basedir)
                        .resolve(uri, true)
                        .changeExtension('', '.md');
