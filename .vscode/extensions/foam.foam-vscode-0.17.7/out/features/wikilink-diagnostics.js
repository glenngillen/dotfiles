...
const FIND_IDENTIFER_COMMAND = {
    ...
    execute: async ({ target, amongst, range }) => {
        if (vscode.window.activeTextEditor) {
            let identifier = workspace_1.FoamWorkspace.getShortestIdentifier(target.path, amongst.map(uri => uri.path));
        ...
        for (const link of resource.links) {
            if (link.type === 'wikilink') {
                ...
                if (section && targets.length === 1) {
                    ...
                    if (utils_1.isNone(note_1.Resource.findSection(resource, section))) {
