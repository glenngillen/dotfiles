...
const findAllFiles = util_1.promisify(glob_1.glob);
/**
...
class Matcher {
    ...
    match(files) {
        const matches = micromatch_1.default(files.map(f => f.toFsPath()), this.include, {
            ignore: this.exclude,
            nocase: true,
...
class FileDataStore {
    ...
    async read(uri) {
        try {
            return (await fs_1.default.promises.readFile(uri.toFsPath())).toString();
        }
        catch (e) {
