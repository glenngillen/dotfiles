...
describe('getDailyNotePath', () => {
    ...
    test('Adds the root directory to relative directories', async () => {
        ...
        const expectedPath = vsc_utils_1.fromVsCodeUri(vscode_1.workspace.workspaceFolders[0].uri).joinPath(config, `${isoDate}.md`);
        ...
        expect(dated_notes_1.getDailyNotePath(foamConfiguration, date).toFsPath()).toEqual(expectedPath.toFsPath());
        await vscode_1.workspace
            .getConfiguration('foam')
            .update('openDailyNote.directory', oldValue);
    });
    test('Uses absolute directories without modification', async () => {
        const config = platform_1.isWindows
            ? 'C:\\absolute_path\\journal'
            : '/absolute_path/journal';
        const expectedPath = platform_1.isWindows
            ? `${config}\\${isoDate}.md`
            : `${config}/${isoDate}.md`;
        const oldValue = await vscode_1.workspace
            .getConfiguration('foam')
        ...
        await vscode_1.workspace
            ...
            .update('openDailyNote.directory', config);
        const foamConfiguration = vscode_1.workspace.getConfiguration('foam');
        expect(dated_notes_1.getDailyNotePath(foamConfiguration, date).toFsPath()).toMatch(expectedPath);
        await vscode_1.workspace
            .getConfiguration('foam')
            .update('openDailyNote.directory', oldValue);
    });
